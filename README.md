...
|---|---------|---------------|-------------------|-----------------|
| 1 | **Income vs Churn**<br>`Box plot` | Распределение дохода у оставшихся (Retained) и ушедших (Churned) | Сравнить медианы и разброс: Retained ≈55k, Churned ≈38k | Клиенты с низким доходом уходят чаще → предложить скидки или продление триала |
| 2 | **Plan vs Churn**<br>`Stacked bar` | Число клиентов по тарифам + доля оттока в каждом | Искать тарифы с наибольшим % оттока (а не абсолютным числом) | В премиум-планах % оттока может быть выше → проверить цену/функционал |
| 3 | **Distribution of Income**<br>`Histogram + KDE` | Плотность распределения дохода всех пользователей | Пик ~60k, лёгкий левый сдвиг | Большинство клиентов зарабатывают 50–70k → ориентир для ценообразования |
| 4 | **User Activity**<br>`Log scale` | Распределение событий на пользователя (логарифмическая шкала) | Большинство <10 событий; длинный хвост сверхактивных | Повышать вовлечённость: пуши, онбординг, бонусы за действия |
| 5 | **Correlation Matrix**<br>`Heatmap` | Корреляции: income, events, purchases | events↔purchases >0.8; income слабо связан | Стимулировать любые действия пользователя → рост покупок и выручки |
| 6 | **Events vs Churn**<br>`Box plot` | Количество событий: у оставшихся и ушедших | Медиана у Retained 10–12 событий, у Churned 5–6 | Низкая активность = сигнал оттока → авто-детект и возврат «затихающих» |
| 7 | **ROC‑Curve**<br>`Model Quality` | Качество моделей предсказания оттока (TPR vs FPR) | LR и RF: AUC≈0.5 (не работают); GB/XGB: целевые AUC>0.82 | На синтетике отток непредсказуем; в реале нужны качественные фичи |
| 8 | **Feature Importance**<br>`Random Forest` | Важность признаков в модели | Топ: income, events, purchases; регионы не важны | При сборе данных приоритет — поведенческие признаки, а не демография |
| 9 | **Confusion Matrix**<br>`Random Forest` | TP/TN/FP/FN для модели | Recall=9% (пропущено 330 из 3646 ушедших) | Оптимизировать под recall — лучше ложная тревога, чем пропустить отток |
| 10 | **Model Metrics Table**<br>`4 модели` | AUC/Accuracy/Precision/Recall/F1 | LR/RF провалились (AUC~0.5); GB/XGB — эталон (AUC>0.82) | В продакшене использовать градиентный бустинг на реальных данных |
| 11 | **Cumulative LTV**<br>`By Cohort` | LTV по когортам регистрации (по месяцам) | Старые когорты (2025-05) имеют бόльший LTV, чем новые | Качество привлечения падает → аудит маркетинговых каналов и онбординга |
| 12 | **Rolling ARPU**<br>`Daily + SMA7` | Ежедневный ARPU + 7-дневное скользящее среднее | Нисходящий тренд, подтверждённый скользящим средним | Монетизация снижается → исследовать частоту покупок и средний чек |
| 13 | **Churn Dynamics**<br>`Daily Count` | Ежедневное количество завершённых подписок | Волатильность: пики до 400/день, в среднем 100–200 | Пики могут совпадать с окончанием триала → добавить напоминание за 3 дня |
...
| График | Что показывает | Как анализировать | 💡 Бизнес-вывод |
|--------|---------------|-------------------|-----------------|
| **DAU**<br>`Daily Active Users` | Уникальные активные пользователи в день | Стабильно ~8 000; провал 29 мая (неполный день) | ~8% активации — нормально; исключать неполные дни из аналитики |
| **ARPU**<br>`Avg Revenue Per User` | Средняя выручка на пользователя в день | Тренд на снижение, подтверждён скользящим средним | Тревожный сигнал по монетизации → проверить конверсию и средний чек |
